		...
		function process() {
			...
			while (currentStack.length) {
				...
				if (!(fnToCall instanceof Function)) {
					if (currentState == State.FULFILLED) promiseToFulFill.resolve(currentOutput);
					else if (currentState == State.REJECTED) promiseToFulFill.reject(currentOutput);
					return;
				}
				//setImmediate(function () {
				try {
					var output = fnToCall(currentOutput);
					console.log(output);
